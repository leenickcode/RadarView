    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child.getId() == R.id.id_scan_circle) {
                //为雷达扫描图设置需要的属性
                Log.d(TAG, "onMeasure: 为雷达扫描图设置需要的属性 data=");
                ((RadarView) child).setScanningListener(this);
                //考虑到数据没有添加前扫描图在扫描，但是不会开始为CircleView布局
//                if (mDatas != null && mDatas.size() > 0) {
//                    Log.d(TAG, "onMeasure: 为雷达扫描图设置需要的属性 data=" + mDatas.size());
...
//        removeAllViews();
        ...
        ((RadarView) getChildAt(0)).setMaxScanItemCount(mDatas.size());
        ((RadarView) getChildAt(0)).startScan();
